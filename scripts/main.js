 ...
 const pokemonFilteredByType = document.getElementById("pokemon-filtered-by-type")
...
const search = document.querySelector("#name").value;
  ...
  const requestPkm = await fetch(`https://pokeapi.co/api/v2/pokemon/pikachu`);
...
function displayPokemonData(data, evolutions) {
  ...
  `
  const markup = document.createRange().createContextualFragment(pokemonTemplate).children[0];
